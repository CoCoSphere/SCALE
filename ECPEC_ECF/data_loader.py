...
class PrecomputedFeatureMECPEDataset(Dataset):
    """使用离线缓存的句向量进行训练/评估的 MECPE 数据集。
    仍然从 JSON 读取 speakers/emotions/pairs，用缓存替代 tokenizer+RoBERTa。
    """
    def __init__(self, json_file_path, feature_file_path, dataset_type='iemocap',
                 max_doc_len=35, pred_future_cause=True, use_emocate=False,
                 use_emotion_category=True, negative_sampling_ratio=1.0,
    ...
    def _prepare_dialogue(self, data_item):
        ...
        if pair_labels.numel() > 0 and self.negative_sampling_ratio and self.negative_sampling_ratio > 0:
            pos_mask = (pair_labels == 1)
            neg_mask = (pair_labels == 0)
            pos_idx = _torch.nonzero(pos_mask, as_tuple=False).view(-1)
            neg_idx = _torch.nonzero(neg_mask, as_tuple=False).view(-1)
            if pos_idx.numel() > 0 and neg_idx.numel() > 0:
                k = int(round(self.negative_sampling_ratio * pos_idx.numel()))
                k = max(0, min(k, neg_idx.numel()))
                if k > 0:
                    perm = _torch.randperm(neg_idx.numel())
                    sampled_neg = neg_idx[perm[:k]]
                    keep_idx = _torch.cat([pos_idx, sampled_neg], dim=0)
                else:
                    keep_idx = pos_idx
            elif pos_idx.numel() == 0 and neg_idx.numel() > 0:
                N = min(200, neg_idx.numel())
                perm = _torch.randperm(neg_idx.numel())
                keep_idx = neg_idx[perm[:N]]
            else:
                keep_idx = _torch.arange(pair_labels.numel())
            pair_indices = pair_indices[keep_idx]
            pair_distances = pair_distances[keep_idx]
            pair_labels = pair_labels[keep_idx]
        ...
        return {
            ...
            'texts': texts,  # 供可能的 doc 编码使用，不会被读取
            'conversation_id': conv_id
        }
...
def collate_fn_features(batch):
    import torch as _torch
    input_feats = _torch.stack([item['precomputed_features'] for item in batch])  # (B, L, D)
...
def create_feature_data_loaders(dataset_name='iemocap', feature_dir='MECPE/features', batch_size=8,
                               pred_future_cause=True, use_emocate=False, use_emotion_category=True,
                               negative_sampling_ratio=1.0, eval_max_pair_distance=None, train_max_pair_distance=None,
                               max_doc_len=50):
    # 路径
    ...
    if train_max_pair_distance is not None:
        train_dataset = PrecomputedFeatureMECPEDataset(
    ...
    test_dataset = PrecomputedFeatureMECPEDataset(
    ...
    vocab_size = 0  # 训练阶段不再需要 tokenizer 词表
    ...
    dev_loader = None
    if dev_json:
        dev_dataset = PrecomputedFeatureMECPEDataset(
